...
public class RegistrationController {
	...
	@GetMapping("/{id}")
	public String showDetailsRegistrationForm(@PathVariable Long id, 
			Model model,
			HttpServletRequest request) {
    	System.out.println();
    	Registration registration = registrationService.getRegistrationById(id).get();
    	
    	model.addAttribute("registration", registration);
		
		request.getSession().setAttribute("registrationSession", registration);
		
		System.out.println("updating registration id="+ id);
		return "/registrations/registration-details";
	}
	...
	public String showUpdateRegistrationForm(@PathVariable Long id, 
			Model model,
			HttpServletRequest request) {
    	System.out.println();
    	Registration registration = registrationService.getRegistrationById(id).get();
	...
	public String updateRegistration(@Valid @ModelAttribute("registration") Registration registration,
       ...
       System.out.println("updating the registration ID ="+ registration.getId());
		Registration registrationExistent = registrationService.getRegistrationById(registration.getId()).get();
		
		
		registrationExistent.setStudent(registration.getStudent());
		registrationExistent.setWorkshop(registration.getWorkshop());
		
		registrationService.updateRegistration(registrationExistent);
		
		
        redirectAttributes.addFlashAttribute("updated", "Registration updated!");
		return "redirect:/registration/all";
	...
	//selecting a workshop and going back to the workshop-add page...
	...
	public String selectWorkshopProcess(@PathVariable("id") int id, Model model, HttpServletRequest request) {
		...
		System.out.println("Workshop selected is..."+ registrationSession.getWorkshop().getDescription());
